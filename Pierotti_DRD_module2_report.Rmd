...
```{r, message=FALSE, warning=FALSE}
RGSet <-
  suppressWarnings(read.metharray.exp(base = './Input_data', targets = targets))
dim(RGSet)
```
...
The red lines represent the DS samples beta/M mean densities while the blue line represents that of the `WT` samples.
...
DS seems confined to a region where PC1 is greater than PC2, while `WT` is confined in a region where PC2 is greater than PC1.
...
I also slice the data frame to exclude p-values and retain only beta-values.
...
I will also try to use Pearson and Spearman correlation coefficients as distance metrics, instead of euclidean distance.
...
I can now plot the delta against the $-\log_{10}$ of the corrected p-values.
I will use the `BH` p-values here.
...
We can see how the p-values are binned at discrete levels (a consequence of the Mann-Whitney test) and nothing reaches the significance threshold of 0.05.
...
To test this hypothesis I will here repeat the analysis by using a Welch test and without changing the normalization procedure.
