...
plot(pca_results$x[,1], pca_results$x[,2],cex=2,pch=2,col=groups,xlab="PC1",ylab="PC2")
legend("bottomright",legend=levels(groups),col=c(1:nlevels(groups)),pch=2)
```
...
plot(pca_results$x[,1], pca_results$x[,2],cex=2,pch=2,col=slides,xlab="PC1",ylab="PC2")
legend("bottomright",legend=levels(slides),col=c(1:nlevels(slides)),pch=2)
```
...
```{r}
beta_rawp_df <- data.frame(beta_norm, pval_mw)
beta_rawp_df_sorted <- beta_rawp_df[order(beta_rawp_df$pval_mw),]
...
colnames(annotation)
df_manhattan <- merge(df_manhattan, annotation,by="Name")
colnames(df_manhattan)
```
...
```{r}
chr_vect <- factor(df_manhattan$chr,levels=c("chr1","chr2","chr3","chr4","chr5","chr6","chr7","chr8","chr9","chr10","chr11","chr12","chr13","chr14","chr15","chr16","chr17","chr18","chr19","chr20","chr21","chr22","chrX","chrY"))
input_manhattan <- data.frame(chr_vect, df_manhattan$pos, df_manhattan$BH_p)
...
Just for completeness, here I repeat the volcano plot and the manhattan plot by using the t-test instead of the Mann-Whithney test.
